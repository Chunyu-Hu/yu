...
# base_sublevel is the kernel version we're starting with and patching # on top of -- for example, 2.6.22-rc7-git1 starts with a 2.6.21 base, # which yields a base_sublevel of 21.
...
%endif
%if %{with_perf}
BuildRequires: elfutils-libelf-devel elfutils-devel zlib-devel binutils-devel newt-devel python-devel audit-libs-devel perl(ExtUtils::Embed) bison flex
...
Source38: kernel-abi-whitelists.tar.bz2

Source50: config-x86_64-generic
Source51: config-s390x-rhel
Source52: config-nodebug
Source53: config-powerpc64-debug
Source54: config-generic
Source55: config-ia64-generic-rhel
Source56: config-i686-debug
Source57: config-i686-nodebug
Source58: config-powerpc64
Source59: config-s390x-kdump-rhel
Source60: config-framepointer
Source61: config-powerpc-generic
Source62: config-x86_64-nodebug
Source63: config-generic-rhel
Source64: config-x86-generic-rhel
Source65: config-s390x-kdump
Source66: config-x86_64-generic-rhel
Source67: config-s390x-debug
Source68: config-i686-debug-rhel
Source69: config-x86_64-debug
Source70: config-powerpc64-kdump
Source71: config-x86_64-nodebug-rhel
Source72: config-s390x
Source73: config-i686-nodebug-rhel
Source74: config-powerpc64-rhel
Source75: config-x86_64-debug-rhel
Source76: config-debug-rhel
Source77: config-debug
Source78: config-i686-rhel
Source79: config-i686
Source80: config-powerpc-generic-rhel
Source81: config-x86-generic
Source82: config-nodebug-rhel
Source83: config-powerpc64-kdump-rhel
...
%global perf_make \
  make %{?_smp_mflags} -C tools/perf -s V=1 prefix=%{_prefix}
%if %{with_perf}
%{perf_make} all
